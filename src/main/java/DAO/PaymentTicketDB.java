...
public class PaymentTicketDB {
    public static boolean insertPaymentTicket(PaymentTicket paymentTicket) {
        ...
        try {
            ...
            String sql = "INSERT INTO Payment (account_id, payment_date, payment_method, payment_status) " +
                         "VALUES (?, ?, ?, ?)";
            pstmt = conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS);

            pstmt.setInt(1, paymentTicket.getAccountId());
            pstmt.setDate(2, java.sql.Date.valueOf(paymentTicket.getPaymentDate()));
            pstmt.setString(3, paymentTicket.getPaymentMethod());
            pstmt.setString(4, paymentTicket.getPaymentStatus());
